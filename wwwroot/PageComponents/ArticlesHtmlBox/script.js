...
export class ArticlesHtmlBox {
    ...
    constructor(target, position, search, page) {
        this._Target = target;
...
<li>\
    ...
    async _ApiAticles() {
        const response = await fetch(this._Page.UrlApi + "/RtInk/Articles?search=" + this._Search + "&take=" + this._Take + "&page=" + this.Page, {
            method: "GET",
    ...
    async _ApiArticleBody(titleHb) {
        const response = await fetch(this._Page.UrlApi + "/RtInk/ArticleBody?titleHb=" + titleHb.toString(), {
            method: "GET",
            headers: { "Accept": "application/json", "Authorization": "Bearer " + this._Page.Session.Token }
        });
        if (response.ok == true) return await response.json();
