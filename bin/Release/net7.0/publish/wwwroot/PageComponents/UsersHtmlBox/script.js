export class UsersHtmlBox {
    ...
    async AppendList() {
        let list = await this._ApiGetList()
        ...
        list.forEach(e => {
            document.querySelector("#" + this.Name + " > ul").insertAdjacentHTML("beforeend", this._ItemHtmlBox(e.login, e.artn, e.title))
            if (this._SessionRoleId > 0)
                this._SubscribButtonItemHtmlBox(e.login, e.subscribeType)
        })
    ...
    _ItemHtmlBox(login, articleN, title) {
        let href = "/i/-" + login
    ...
    _SubscribButtonItemHtmlBox(login, subscribeType) {
        let html = "\
        ...
        trg.addEventListener('click', async (event) => {
            let usb = await this._ApiUserSubscrib(login)
            if (usb.ok) {
                if (subscribeType == 0) {
                    subscribeType = 2
